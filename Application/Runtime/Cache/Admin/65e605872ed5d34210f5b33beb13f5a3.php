...
<body class="childrenBody">
    ...
    <input type="text" name="name" autocomplete="off" class="layui-input" placeholder="用户名"/>
</div><div class="layui-input-inline" style="width: 16%">
    <input type="text" name="phone" autocomplete="off" class="layui-input" placeholder="移动电话"/>
</div><div class="layui-input-inline" style="width: 13%">
    <select title="可用状态" name="state">
...
{field:'id',title:'ID',align:'center',width:80},{field:'name',title:'用户名',align:'center'},{field:'phone',title:'移动电话',align:'center'},{field:'birdate',title:'生日',align:'center'},{field:'headimage',title:'头像',align:'center',templet:function(d){
                return '<div><img height="28" src="'+d.headimage+'"></div>';
            }},{field:'state',title:'可用状态',align:'center',templet:function(d){
                if(d.state == '0') return '禁用';
...
{title: '操作',templet:'#tool', width:130,fixed: "right", align: "center"}
        ...
        $(".delAll_btn").on('click',function () {
            ...
            if (data.length > 0) {
                ...
                layer.confirm('确定删除选中的数据？', {icon: 3, title: '提示信息'}, function (index) {
                    $.post("/admin/User/delete",{'id':ids},function(data){
        ...
        table.on('tool(table)', function (obj) {
            let layEvent = obj.event,
                data = obj.data;
            if (layEvent === 'edit') { //编辑
                addoredit('/admin/User/edit/id/'+data.id,'修改');
            } else if (layEvent === 'del') { //删除
                layer.confirm('确定删除这条数据？', {icon: 3, title: '提示'}, function (index) {
                    layer.close(index);
                    $.post("/admin/User/delete",{'id':data.id},function(data){
                        window.parent.toast(data.info,data.code);
                        if(data.code === 0)
                            tablist.reload();
                    });
